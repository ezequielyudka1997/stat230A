...
PAP: Public assistance income past 12 months (use ADJINC to adjust to
constant dollars)
0 .None
...
WAGP: Wages or salary income past 12 months (use ADJINC to adjust WAGP to
constant dollars)
0 .None
...
```
```{r}
model1 = lm(log(adj_WAGP) ~ as.factor(RAC1P) + AGEP + as.factor(ST) + Years_in_US + DIS, data = immigrants_only2)
```
```{r}
...
MR: Differences across earnings for immigrants of different Races

MR: Differences across earnings for Asian immigrants from countries (at least 10%)
