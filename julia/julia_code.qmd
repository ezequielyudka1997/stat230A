...
```{r}
model1 = lm(log(adj_WAGP) ~ as.factor(RAC1P) + AGEP + Years_in_US + DIS + as.factor(ENG) + SEX + wb50birth + Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + Citizen + as.factor(MAR), data = immigrants_only2)
```
...
```{r}
model2 = lm(log(adj_WAGP) ~  AGEP + Years_in_US + DIS + as.factor(ENG) + SEX + wb50birth + Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + Citizen + as.factor(POBP) + as.factor(MAR), data = asian_only)
```
...
```{r}
model3 = lm(log(adj_WAGP) ~ as.factor(RAC1P) +  AGEP + Years_in_US + DIS + as.factor(ENG) + SEX + wb50birth + Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + Citizen + as.factor(POBP) + as.factor(MAR), data = hisp_only)
```
