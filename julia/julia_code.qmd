...
```{r}
wagecit<- ggplot(processed_data, aes(x = WAGP, fill = factor(Citizen), color = factor(Citizen))) +
...
```
```{r}
logwagecit<- ggplot(processed_data, aes(x = log(adj_WAGP), fill = factor(Citizen), color = factor(Citizen))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
```{r}
wagerace<- ggplot(processed_data, aes(x = WAGP, fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
```
```{r}
logwagerace<- processed_data%>%filter(RAC1P %in% c(1,2,6))%>%ggplot(aes(x = log(adj_WAGP), fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
  facet_wrap(vars(factor(Citizen))) +
...
```
```{r}
logwage_for<- processed_data%>%filter(RAC1P %in% c(1,2,6,8,9))%>%ggplot(aes(x = log(adj_WAGP))) +
...
```{r}
model0 = lm(log(adj_WAGP) ~as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + PAP + HINS4 + immigrant, data = processed_data)
```
...
```{r}
model1 = lm(log(adj_WAGP) ~ as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + as.factor(MAR) + PAP + HINS4, data = immigrants_only2)
```
...
density_plot_rac1p <- ggplot(immigrants_only2 %>% filter(RAC1P %in% c(1, 2, 6)), aes(x = log(adj_WAGP), fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
# Create density plot for RAC1P groups
density_plot_rac1p2 <- ggplot(processed_data %>% filter(RAC1P %in% c(1, 2, 6) & immigrant ==0), aes(x = log(adj_WAGP), fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
density_plot_asian <- ggplot(asian_only_im, aes(x = log(adj_WAGP), fill = factor(POBP), color = factor(POBP))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
  labs(title = "Distribution of Immigrant Wages for Asians by Country of Origin",
       x = "Wages",
       y = "Density (Proportion)")+ ylim(0,0.6)

density_plot_asian2 <- ggplot(asian_only, aes(x = log(adj_WAGP))) +
...
```{r}
model2 = lm(log(adj_WAGP) ~ AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + PAP + HINS4 + as.factor(MAR) , data = asian_only_im)
```
...
```{r}
model2.1 = lm(log(adj_WAGP) ~ AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + PAP + HINS4 + immigrant + as.factor(MAR) , data = asian_only)
```
...
```{r}
# Create density plot for RAC1P groups
density_plot_rac1p <- ggplot(hisp_only_im, aes(x = log(adj_WAGP), fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
# Create density plot for RAC1P groups
density_plot_rac1p2 <- ggplot(hisp_only %>% filter(immigrant ==0), aes(x = log(adj_WAGP), fill = factor(RAC1P), color = factor(RAC1P))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
...
density_plot_hisp <- ggplot(hisp_only_im, aes(x = log(adj_WAGP), fill = factor(POBP), color = factor(POBP))) +
  geom_density(aes(y = ..density..), alpha = 0.5) +
  labs(title = "Distribution of Immigrant Wages for Hispanics by Country of Origin",
       x = "Wages",
       y = "Density (Proportion)")+ ylim(0,0.6)

density_plot_hisp2 <- ggplot(hisp_only%>% filter(immigrant ==0), aes(x = log(adj_WAGP))) +
  geom_density(aes(y = ..density..), alpha = 0.5, alpha = 0.5, fill = "cornflowerblue", color = "cornflowerblue") +
  labs(title = "Distribution of non-Immigrant Wages for Hispanics",
       x = "Wages",
       y = "Density (Proportion)")+ ylim(0,0.6)

density_plot_hisp3 <- ggplot(hisp_only_im, aes(x = log(adj_WAGP))) +
  geom_density(aes(y = ..density..), alpha = 0.5, alpha = 0.5, fill = "cornflowerblue", color = "cornflowerblue") +
...
```{r}
model3 = lm(log(adj_WAGP) ~ as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + PAP + HINS4 + as.factor(MAR), data = hisp_only_im)
```
...
```{r}
model3.1 = lm(log(adj_WAGP) ~ as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + PAP + HINS4 + immigrant + as.factor(MAR), data = hisp_only)
```
