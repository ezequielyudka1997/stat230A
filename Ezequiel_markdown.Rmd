...
```{r}
employment_status_model <- glm(Unemployed ~  as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + as.factor(MAR) , data = processed_data, family = binomial())
...
```
```{r}
filtered_results$Variable <- rownames(filtered_results)
ggplot(filtered_results, aes(x = Variable, y = Coefficients)) +
  geom_point() +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2) +
  coord_flip() +
  labs(title = "Coefficient plot with Confidence Intervals",
       x = "Variable",
       y = "Exponential Coefficients") +
  theme_minimal()
print(ggplot)
```
...
Hispanic immigrants
```{r}
hisp_only_employment_status_model <- glm(Unemployed ~  as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), data = hisp_only_im, family = binomial())
...
```{r}
hisp_only_poverty_risk_model <- glm(PAP_binary ~ as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), family = binomial(), data = hisp_only_im)
...
```{r}
hisp_only_medicaid_model <- glm(HINS4 ~ as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), family = binomial(), data = hisp_only_im)
...
```{r}
asian_only_employment_status_model <- glm(Unemployed ~  as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), data = asian_only_im, family = binomial())
...
```{r}
asian_only_poverty_risk_model <- glm(PAP_binary ~ as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), family = binomial(), data = asian_only_im)
...
```{r}
asian_only_medicaid_model <- glm(HINS4 ~ as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), family = binomial(), data = asian_only_im)
...
variances <- diag(v)
std_errors <- sqrt(diag(v))
coefficients <- coef(asian_only_medicaid_model)
exp_coefficients <- exp(coefficients)
lower_bound <- exp_coefficients * exp(-1.96 * std_errors)
upper_bound <- exp_coefficients * exp(1.96 * std_errors)
results <- data.frame(
...
ggplot(all_coefficients, aes(x = Variable, y = Coefficient, fill = Model)) +
  ...
  labs(title = "Comparison of Coefficients: Regression A vs. B",
       x = "Variables",
       y = "Coefficients") +
  theme_minimal() +
