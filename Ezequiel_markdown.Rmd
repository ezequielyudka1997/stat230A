...
largest_subgroup = hisp_only_im%>%group_by(POBP)%>%
  summarise(count = n())%>%largest_subgroup
...
```{r}
asian_only_medicaid_model <- glm(HINS4 ~ as.factor(POBP) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + as.factor(MAR), family = binomial(), data = hisp_only_im)
