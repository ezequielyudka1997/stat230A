...
generate_and_save_plots <- function(data, variable, file_name) {
  p1 <- ggplot(data, aes_string(x = variable)) +
    geom_bar(fill = "blue") +
    labs(title = paste("General Distribution of", variable), x = variable, y = "Count")
  
  p2 <- ggplot(data, aes_string(x = variable)) +
    geom_bar(fill = "blue") +
    facet_grid(. ~ Citizen) +
    labs(title = paste("Distribution of", variable, "by Citizen Status"), x = variable, y = "Count")
  
  p3 <- ggplot(data, aes_string(x = variable)) +
    geom_bar(fill = "blue") +
    facet_grid(. ~ immigrant) +
    labs(title = paste("Distribution of", variable, "by Immigrant Status"), x = variable, y = "Count")
  
  combined_plot <- grid.arrange(p1, p2, p3, ncol = 1)
  
  # Save the combined plot to a file
  ggsave(file_name, combined_plot, width = 8, height = 18)
}
