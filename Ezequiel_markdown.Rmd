...
as.factor(RAC1P) + AGEP + SEX*Citizen + as.factor(ENG) + SEX*DIS + wb50birth+ Unemployed + as.factor(ST) + High_School_Grad + Bachelors_Degree + HISP_dummy + as.factor(MAR) + PAP + HINS4
...
variances <- diag(v)
high_variance_indices <- variances > 0.05
high_variances <- variances[high_variance_indices]

if (length(high_variances) > 0) {
    print("Variances higher than 0.05:")
    print(high_variances)
} else {
    print("No variances higher than 0.05.")
}
```
